...
const PRESET_CATEGORIES = [
  { label: 'Eventos',   icon: '🎪', description: 'Palestras, workshops, feiras' },
  { label: 'Palestras', icon: '🎤', description: 'Apresentações e talks' },
  { label: 'Aulas',     icon: '📖', description: 'Aulas particulares ou grupos' },
  { label: 'Livros',    icon: '📚', description: 'Venda ou royalties de livros' },
  { label: 'Outro',     icon: '✏️', description: 'Outra receita' },
]
...
export default function NovaReceitaPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isOutro && !customDescription.trim()) { setError('Descreva a receita'); return }
    if (!amount) { setError('Informe o valor'); return }

    setLoading(true)
    setError('')

    const { error: err } = await supabase.from('other_income').insert({
      date,
  ...
  return (
    <div className="px-4 pt-12 pb-8">
      ...
      <h1 className="font-display text-2xl font-bold text-brand-charcoal mb-1">Nova receita</h1>
